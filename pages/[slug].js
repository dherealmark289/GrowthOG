...
export default function BlogPost({ post, relatedPosts }) {
  ...
  const structuredData = {
    ...
    'mainEntityOfPage': {
      '@type': 'WebPage',
      '@id': `https://growthog.com/${post.slug}/`
    }
  };
  ...
  return (
    <Layout
      seo={{
        ...
        description: seoDescription,
        // Use direct slug URL for canonical
        canonical: `https://growthog.com/${post.wpSlug || post.slug}/`,
        openGraph: {
          title: post.title,
          description: seoDescription,
          type: 'article',
          url: `https://growthog.com/${post.wpSlug || post.slug}/`,
          images: [
            {
              ...
              <div className="mb-6 bg-white p-4 rounded-lg shadow-sm">
                <div className="flex items-center">
                  ...
                  <div className="flex space-x-2">
                    <a href={`https://www.facebook.com/sharer/sharer.php?u=${encodeURIComponent(`https://growthog.com/${post.wpSlug || post.slug}/`)}`} 
                    ...
                    <a href={`https://twitter.com/intent/tweet?text=${encodeURIComponent(post.title)}&url=${encodeURIComponent(`https://growthog.com/${post.wpSlug || post.slug}/`)}`}
                       target="_blank" rel="noopener noreferrer"
                    ...
                    <a href={`https://www.linkedin.com/sharing/share-offsite/?url=${encodeURIComponent(`https://growthog.com/${post.wpSlug || post.slug}/`)}`}
                       target="_blank" rel="noopener noreferrer"
                    ...
                    <a href={`mailto:?subject=${encodeURIComponent(post.title)}&body=Check out this article: ${encodeURIComponent(`https://growthog.com/${post.wpSlug || post.slug}/`)}`}
            ...
            <div className="md:w-1/4 lg:w-1/5">
              <div className="md:sticky md:top-24 space-y-8">
                ...
                {relatedPosts && relatedPosts.length > 0 && (
                  <div className="bg-white rounded-lg shadow-sm p-6">
                    <h3 className="text-xl font-semibold mb-4 text-secondary-900">Related Articles</h3>
                    <div className="space-y-4">
                      {relatedPosts.map((related, index) => (
                        <div key={index} className="group">
                          <Link href={`/${related.wpSlug || related.slug}`}>
                            <h4 className="text-secondary-800 font-medium hover:text-primary-600 transition-colors line-clamp-2 group-hover:text-primary-600">
                              {related.title}
                            </h4>
                          </Link>
...
export async function getStaticPaths() {
  try {
    // Fetch first 100 posts for pre-rendering
    const { posts } = await fetchBlogPosts(1, 100);
    
    // Create paths for each post using direct slug
    const paths = posts.map((post) => {
      const formattedPost = formatWordPressPost(post);
      return {
        params: { 
          slug: formattedPost.wpSlug || formattedPost.slug 
        }
      };
    });
    ...
    return {
      paths,
      // Enable fallback for posts not included in paths
      fallback: true,
    };
  } catch (error) {
